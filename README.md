...
Two optional switches on the home page give your post extra reach before you even hit generate:

- 🔍 **Search keyword embedding (SEO)**: set 1-3 target search keywords and the outline, title, body, and tags will weave them in naturally (title within the first 15 characters, body within the first 80, tags mixing core and long-tail terms) — the pre-publish checklist also adds three SEO traffic-light checks
- 🧬 **Batch variants**: generate 2-5 differentiated outlines in one go — varied hooks, angles, and formats — saved straight to drafts, built for Douyin image-post volume publishing
...
|-------|------|-------------|
| **Ideate** | Topic Inspiration | Enter your niche; AI generates topic ideas with angles, content formats, and estimated heat. Toggle "Use my account data" to inject real performance data logged in Analytics Review; selected topics carry their angle and suggested tags into the creation flow. A "My Topic Library" tab keeps every saved idea in an idea → todo → done → viral status flow — collect topics from the generator or Comment Insights, or add your own by hand. "Trending mode" turns pasted official trending keywords into an angle for your niche, a suggested publish window, and a niche-relevance rating. "Expand into series" splits one theme into 5-10 progressive episodes (auto-named "Series \| 0X") that can be batch-saved to the library and optionally scheduled into the content calendar daily / every other day / weekly |
| | Benchmark Breakdown | Paste a viral post or a web link to deconstruct why it worked (hook/structure/emotion/audience) and generate an original draft in the same pattern; a "Clip inbox" collects posts captured with the browser clipper extension (see below) for one-click breakdown; dissected structure templates can be saved to a "Pattern library" (up to 50) and reused anytime; "Send to creation" now splits the draft into an outline locally at zero AI cost, with AI re-arrangement kept as a secondary option; the last 20 breakdowns are archived locally |
...
| **Write** | Viral Titles | Generate candidate titles with attractiveness scores and viral-element tags; your current topic is prefilled automatically, and any candidate can be set as the work's title or used to start a new creation in one click; past batches are archived locally |
| | Multi-Platform Rewrite | Rewrite one piece of copy into Xiaohongshu / Douyin / WeChat / Bilibili / Weibo styles; send any rewrite to the creation center as an outline in one click — pages are split locally at zero AI cost; past rewrites are archived locally |
| | Comment Assistant | Paste fan comments to generate high-engagement replies and a pinned guiding comment; replies follow your brand persona (catchphrases and banned words respected), the same batch of comments can be handed to Comment Insights and back without re-pasting, and results are archived locally |
| | Video Script | Turn a post into a 30s / 60s / 3min short-video narration script with a timestamped storyboard (hook, visual cues, spoken lines, subtitle breaks, CTA); supports brand persona injection, you can copy the full script or any single line, and past scripts are archived locally |
| **Style & Visuals** | Brand Memory | Manage personal IP / brand profiles (tone, catchphrases, signature, banned words) for a consistent persona; new creators can run the 3-step positioning wizard — who you are / who it's for / why you — and AI drafts the brand profile plus a starter list of 10 topics |
...
| | Cover A/B | Generate multiple cover directions for one topic and compare estimated click appeal; a chosen direction flows straight into the main pipeline — its title onto the cover page, its visual concept into the style prompt; comparisons are archived locally, and a "mark adopted" action gradually builds your own cover win-rate data |
...
| **Plan & Review** | Content Calendar | Plan publishing cadence across platforms and manage plans and statuses; entries support publish times and linked works ("Add to Calendar" from history links automatically, and "View work" jumps straight to the linked record); "Start creating" on a plan carries its angle and tags into the creation flow; marking a plan as published pops a one-time "golden window" checklist — reply to comments within the first hour, pin a guiding comment via Comment Assistant, backfill the numbers after 48 hours; new entries get a best-time-slot recommendation based on your account data; "AI Scheduling" generates a one-week content plan in one click (optionally using account data, with a preview to check off before saving); a built-in hotspot calendar covers year-round marketing moments (solar and lunar festivals, e-commerce sales, seasonal rules, with 2026–2028 lunar-date mapping), shown as an overlay layer plus an "upcoming hotspots" countdown block, and any hotspot can spawn topic ideas in one click — works without an AI model |
| | Analytics Review | Log published metrics manually, bulk-paste from Excel / CSV (headers auto-detected, numbers like "1.2万" auto-converted), or use "Screenshot import" — OCR your creator-dashboard screenshots through your own multimodal model, with an editable preview before saving (manual entry always available as a fallback); view SVG trend / comparison charts and publishing time-slot analysis; industry-benchmark traffic lights grade your engagement / like / save / comment rates against public reference values and are injected into the AI review insights, whose suggestions now offer one-click "generate topics" and "add to calendar" |
...
## ✂️ Browser Clipper Extension

See a post worth studying while you browse? The bundled clipper (in the `browser-extension/` directory) extracts the title, body, tags, and engagement numbers from a Xiaohongshu (RED) or Douyin web note page in one click, sends it to your local RedInk clip inbox, and from there fills Benchmark Breakdown in one click.

- 🧩 Manifest V3, vanilla JavaScript, zero dependencies — just load it unpacked in Chrome or Edge
- ⚙️ Supports a custom backend port and access token; see [browser-extension/README.md](./browser-extension/README.md) for setup
- 🔒 The clipper reads pages through your own logged-in browser session — use it for personal study and analysis only
...
- **Another version**: if the whole direction feels off, regenerate the entire outline in one click instead of fixing it page by page
- **Blank-page check & cost estimate**: before image generation starts, blank pages are flagged and you see an estimate of how many images the run will cost
- **Save indicator with retry**: a three-state saving / saved / failed indicator keeps you informed, with one-click retry when a save fails
- **Smarter page adding**: a newly added page is focused automatically, with its page type selectable on the spot
...
- **Faster copywriting**: candidate titles, body copy, and tags now generate in parallel with the images, so your publishing package is ready sooner
- **Copy body + tags together**: one combined copy button grabs the body and tags in paste-ready form
- **Add to Calendar**: schedule the finished work into the content calendar right from the result page
- **Overwrite protection**: if you've hand-edited titles / copy / tags, regeneration asks for confirmation before replacing your edits
- **AI content declaration**: the pre-publish checklist includes a permanent reminder to switch on the platform's AI-generated-content declaration (with the exact toggle paths for Xiaohongshu and Douyin), and the same notice sits at the top of the publishing package's text file — platform rules are maintained in `backend/data/platform_rules.json` so they can be updated without code changes
...
- **Iterate on this**: spin any work into a follow-up without starting from scratch — pick a new cover hook, a new angle, a new format, or a sequel
...
### Unreleased
- ✨ Browser clipper extension (Manifest V3, zero dependencies): one-click capture of Xiaohongshu / Douyin web notes into a local clip inbox that feeds Benchmark Breakdown
- ✨ Search keyword embedding (SEO): set 1-3 target keywords on the home page and they are woven into the outline, title, body, and tags, with three SEO traffic-light checks added to the pre-publish checklist
- ✨ Batch variants: generate 2-5 differentiated outlines at once (varied hook / angle / format) straight into drafts, built for Douyin image-post volume publishing
- ✨ Topic Inspiration upgrade: "My Topic Library" with an idea / todo / done / viral status flow, "Trending mode" that turns official trending keywords into angles with suggested publish windows, and "Expand into series" that splits a theme into 5-10 progressive episodes with optional calendar scheduling
- ✨ Benchmark Breakdown upgrade: clip inbox for captured posts, a pattern library of up to 50 reusable structure templates, and "send to creation" now splits pages locally at zero AI cost
- ✨ Analytics Review upgrade: screenshot import via your own multimodal model OCR (with editable preview), industry-benchmark traffic lights injected into AI insights, and one-click "generate topics" / "add to calendar" from AI suggestions
- ✨ Brand Memory 3-step positioning wizard for beginners: answer who you are / who it's for / why you, and AI drafts the brand profile plus 10 starter topics
- ✨ Toolbox interop round: Cover A/B directions flow into the main pipeline (with "mark adopted" win-rate tracking), Comment Assistant gains brand persona injection and shares comment batches with Comment Insights, Viral Titles can set the work title or start a creation, and Rewrite / Titles / Reply / Script / Cover tools all keep local archives
- ✨ Content Calendar upgrade: "Start creating" carries a plan's angle and tags, "View work" jumps to the linked record, and marking a plan published pops a one-time golden-window checklist
- ✨ Outline page upgrade: "another version" full regenerate, blank-page check with cost estimate before generating, a three-state save indicator with retry, and auto-focus on newly added pages with selectable page type
- ✨ Result page upgrade: add to calendar, combined "copy body + tags", copywriting generated in parallel with images, and overwrite confirmation to protect manual edits
- ✨ History "Iterate on this": spin any work into a new cover hook, new angle, new format, or a sequel
- ✨ AI content declaration compliance reminder in the pre-publish checklist and publishing package, with platform rules externalized to `backend/data/platform_rules.json`
- ✨ Access token support for public deployments: set `REDINK_ACCESS_TOKEN` on the backend and enter it under Settings → Access Security
- 🔧 Generation stability hardening: SSE heartbeats, aligned timeouts, clearer error classification, and duplicate-request deduplication
